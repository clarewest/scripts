...
def pdb_buildstructure(pdbfile):
		...
		return pdb_parser.get_structure("name",pdbfile+".pdb")    # This command gets the structure of the PDB
...
if __name__ == '__main__':
	argc = len(sys.argv)
	if argc < 2:
		print "Usage: ./getcontacts <PDBID> Chain 8.0"
	else:
		pdb=sys.argv[1]
		chain=sys.argv[2]
		dist=float(sys.argv[3])
		file_seq = open(pdb+".proxy_fasta", 'w')
		file_map = open(pdb+".proxy_map",'w')
