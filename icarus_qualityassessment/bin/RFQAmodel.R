...
t = read.table("../output/training/Method2_topAll.txt")
...
head(t2)
colnames(t) = c("Protein", "Decoy", "PPV", "MapAlign", "MapLength", "EigenTHREADER", "Contact","SAINT2", "Target_PPV","NumTrue","NumCon","Neff","TMsegment","TMScore","PCons","ProQ2D","ProQRosCenD","ProQRosFAD","ProQ3D","PCombC","Target","Length","SCOP_Class","Beff","NumCon2","S2_Max","S2_Med","S2_Min","S2_Spr","Con_Max","Con_Med","Con_Min","Con_Spr","PPV_Max","PPV_Num","MA_Max","MA_Med","MA_Min","MA_Spr","MA_LEN","ET_Max","ET_Med","ET_Min","ET_Spr","PC_Max","PC_Med","PC_Min","PC_Spr","ProQ2D_Max","ProQ2D_Med","ProQ2D_Min","ProQ2D_Spr","RosCen_Max","RosCen_Med","RosCen_Min","RosCen_Spr","RosFA_Max","RosFA_Med","RosFA_Min","RosFA_Spr","ProQ3D_Max","ProQ3D_Med","ProQ3D_Min","ProQ3D_Spr","PCombC_Max","PCombC_Med","PCombC_Min","PCombC_Spr","TMScore2","Label","Label2")
...
t3 <- subset(t2, Target %in% subset(lst, method=="FM")$Target)
t4 <- merge(t2, lst, by="Target")
...
for (i in c(1,2,3,4,5,6))
{
	...
	plots[[i]]=ggplot(t4,aes_string(x=temp[i],y="TMScore",col="Label"))+geom_point()+facet_wrap(~method)
}
# Define a new plotting window with sensible dimensions:
#dev.new(width=12,height=4)
# Arrange plots in a single row and perform the plotting: 
png(paste("~/pub_html/Clare/RFQA/",DATASET,"models_features.png",sep=""),width=12000,height=4000,res=350)
...
if (FM_FLAG) {
#  val_tab = t3
  val_tab = t3[t3$Protein %in% target_pos_proteins$Protein,]
} else {
...
##### GLM
if (FM_FLAG) {
  png(paste("~/pub_html/Clare/RFQA/ROCCurve",DATASET,"models_FM.png", sep=""),width=2250,height=2250,res=450)
} else {
  png(paste("~/pub_html/Clare/RFQA/ROCCurve",DATASET,"models_ALL.png", sep=""),width=2250,height=2250,res=450)
}
...
fit2 <- randomForest(as.factor(Label) ~ . -Label -Label2 -Protein -Decoy -Target -TMScore -TMScore2 -TMsegment - NumTrue -Target_PPV, data=train_tab, importance=TRUE, ntree=500,mtry = 7)	
...
if (FM_FLAG) {
  png(paste("~/pub_html/Clare/RFQA/ConfusionMatrix",DATASET,"models_FM_",cutoff,".png", sep=""),width=2250,height=2250,res=450)
} else {
  png(paste("~/pub_html/Clare/RFQA/ConfusionMatrix",DATASET,"models_All_",cutoff,".png", sep=""),width=2250,height=2250,res=450)
}
...
write.table(prop, file=paste("~/pub_html/Clare/RFQA/",DATASET,cutoff,"full_details.txt",sep=""), row.names=FALSE, quote=FALSE)
write.table(subset(prop, Set=="All"), file=paste("~/pub_html/Clare/RFQA/",DATASET,cutoff,"brief_details.txt",sep=""), row.names=FALSE, quote=FALSE)
