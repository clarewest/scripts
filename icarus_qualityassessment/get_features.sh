...
#    PPV=$($QA/bin/convert $DECOY_DIR/$DECOY.proxy_fasta $TARGET.aln $TARGET.models.metapsicov.stage1 $DECOY_DIR/$DECOY.proxy_map 2>&1 | grep "^True Positives" | awk '{print $3}');
    PPV=$($QA/bin/convert $DECOY_DIR/$DECOY.proxy_fasta $TARGET.aln $TARGET.models.metapsicov.stage1 $DECOY_DIR/$DECOY.proxy_map 2>&1 | grep "True Positives" | sed 's/.*True Positives: //' | awk '{print $1}');
...
#  python3 $QA/bin/get_RMSDs.py $TARGET . $TARGET --region "flex" --nosuperimpose
  python3 $QA/bin/get_RMSDs.py $TARGET . $TARGET --region "flex" 
...
#  python3 $QA/bin/get_RMSDs.py $TARGET . $TARGET --region "sampled" --nosuperimpose
  python3 $QA/bin/get_RMSDs.py $TARGET . $TARGET --region "sampled" 
