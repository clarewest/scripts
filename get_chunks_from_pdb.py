...
def get_chunk(pdb, chain):
        ...
        chunk = 10
        for i in range(chunk-1,length):
            output = pdb[-8:-4] +"_"+chain+"_"+str(i+start)+".pdb"
            extract(pdb_structure, "A", start, start+i, output)

if __name__ == '__main__':
        argc = len(sys.argv)
        if argc != 3:
                print "Usage: /path/to/get_chunk_from_pdb.py PDB.pdb Chain"
        else:
