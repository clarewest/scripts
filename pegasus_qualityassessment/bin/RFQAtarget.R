...
t = read.table("../output/training/Method1_Training.txt")
...
t[,50] = as.factor(Labels[as.integer(t$V49 >= 0.5) + 1])
t2[,50] = as.factor(Labels[as.integer(t2$V49 >= 0.5) + 1])
...
### Subset of CASP12 targets that were free-modelling ###
...
png(paste("~/pub_html/Clare/RFQA/",DATASET,"target_features.png",sep=""),width=12000,height=4000,res=350)
...
if (FM_FLAG) {
  png(paste("~/pub_html/Clare/RFQA/ROCCurve",DATASET,"target_FM.png", sep=""),width=2250,height=2250,res=450)
} else {
  png(paste("~/pub_html/Clare/RFQA/ROCCurve",DATASET,"target_ALL.png", sep=""),width=2250,height=2250,res=450)
}
...
result4 = calculate_performance(-val_tab$ProQ3D_Max,val_tab$Label,3,"TRUE")
result5 = calculate_performance(-val_tab$PCombC_Max,val_tab$Label,4,"TRUE")
result6 = calculate_performance(-val_tab$PC_Max,val_tab$Label,5,"TRUE")
result7 = calculate_performance(-val_tab$RosFA_Max,val_tab$Label,6,"TRUE")
legend("bottomright",	c(paste("RFQA - AUC=",round(result2[[2]]@y.values[[1]],2)),paste("ProQ2D - AUC=",round(result3[[2]]@y.values[[1]],2)),paste("ProQ3D - AUC=",round(result4[[2]]@y.values[[1]],2)),paste("PCombC - AUC=",round(result5[[2]]@y.values[[1]],2)),paste("PCons - AUC=",round(result6[[2]]@y.values[[1]],2)),paste("ROSETTA FA - AUC=",round(result7[[2]]@y.values[[1]],2))), lwd = 2, lty = 1,col=colours)
dev.off()
...
if (FM_FLAG) {
  png(paste("~/pub_html/Clare/RFQA/ConfusionMatrix",DATASET,"target_FM.png", sep=""),width=2250,height=2250,res=450)
} else {
  png(paste("~/pub_html/Clare/RFQA/ConfusionMatrix",DATASET,"target_ALL.png", sep=""),width=2250,height=2250,res=450)
}
...
missed <- t3 %>% filter(! Target %in% pos_pred_tab2$Target) %>% filter(Label=="Correct")
