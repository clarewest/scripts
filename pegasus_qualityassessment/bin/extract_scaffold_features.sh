...
LRMSD=$(sort -k 21 -n $1.results_filtered75.txt | head -n 1 | awk '{print $21}');
GRMSD=$(sort -k 22 -n $1.results_filtered75.txt | head -n 1 | awk '{print $22}');
FNAT=$(sort -k 25 -n $1.results_filtered75.txt | tail -n 1 | awk '{print $25}');
FNONNAT=$(sort -k 26 -n $1.results_filtered75.txt | head -n 1 | awk '{print $26}');
...
# 12. LocalFLEX
LFLEX_MAX=$(sort -k 24,24nr $1.results_filtered75.txt | tail -n 1 | awk '{print $24}');
# (b) Minimum value:
LFLEX_MIN=$(sort -k 24,24n $1.results_filtered75.txt | tail -n 2 | head -n 1 | awk '{print $24}');
# (c) Median value:
LFLEX_MED=$(sort -k 24,24n $1.results_filtered75.txt | head -n $((LINES / 2)) | tail -n 1 | awk '{print $20}');
# (d) Spread:
LFLEX_SPR=$(echo "$LFLEX_MAX - $LFLEX_MED" | bc)

# 13. GlobalFLEX
GFLEX_MAX=$(sort -k 23,23nr $1.results_filtered75.txt | tail -n 1 | awk '{print $23}');
# (b) Minimum value:
GFLEX_MIN=$(sort -k 23,23n $1.results_filtered75.txt | tail -n 2 | head -n 1 | awk '{print $23}');
# (c) Median value:
GFLEX_MED=$(sort -k 23,23n $1.results_filtered75.txt | head -n $((LINES / 2)) | tail -n 1 | awk '{print $23}');
# (d) Spread:
GFLEX_SPR=$(echo "$GFLEX_MAX - $GFLEX_MED" | bc)

# 14. Local PCons Scores
LOCAL_PCONS_MAX=$(sort -k 27,27n $1.results_filtered75.txt | tail -n 1 | awk '{print $27}');
# (b) Minimum value:
LOCAL_PCONS_MIN=$(sort -k 27,27nr $1.results_filtered75.txt | tail -n 2 | head -n 1 | awk '{print $27}');
# (c) Median value:
LOCAL_PCONS_MED=$(sort -k 27,27n $1.results_filtered75.txt | head -n $((LINES / 2)) | tail -n 1 | awk '{print $27}');
# (d) Spread:
LOCAL_PCONS_SPR=$(echo "$LOCAL_PCONS_MAX - $LOCAL_PCONS_MED" | bc)

# 15. Local ProQ3D Scores
LOCAL_PROQ3D_MAX=$(sort -k 28,28n $1.results_filtered75.txt | tail -n 1 | awk '{print $28}');
# (b) Minimum value:
LOCAL_PROQ3D_MIN=$(sort -k 28,28nr $1.results_filtered75.txt | tail -n 2 | head -n 1 | awk '{print $28}');
# (c) Median value:
LOCAL_PROQ3D_MED=$(sort -k 28,28n $1.results_filtered75.txt | head -n $((LINES / 2)) | tail -n 1 | awk '{print $28}');
# (d) Spread:
LOCAL_PROQ3D_SPR=$(echo "$LOCAL_PROQ3D_MAX - $LOCAL_PROQ3D_MED" | bc)

# OUTPUTTING FEATURES:
echo "$BASIC_INFO $NUM_CON $S2_MAX $S2_MED $S2_MIN $S2_SPR $CON_MAX $CON_MED $CON_MIN $CON_SPR $PPV_MAX $PPV_NUM $MAP_MAX $MAP_MED $MAP_MIN $MAP_SPR $MAP_MHL $EIG_MAX $EIG_MED $EIG_MIN $EIG_SPR $PCONS_MAX $PCONS_MED $PCONS_MIN $PCONS_SPR $PROQ2D_MAX $PROQ2D_MED $PROQ2D_MIN $PROQ2D_SPR $ROSCEN_MAX $ROSCEN_MED $ROSCEN_MIN $ROSCEN_SPR $ROSFA_MAX $ROSFA_MED $ROSFA_MIN $ROSFA_SPR $PROQ3D_MAX $PROQ3D_MED $PROQ3D_MIN $PROQ3D_SPR $PCOMBC_MAX $PCOMBC_MED $PCOMBC_MIN $PCOMBC_SPR $TM $LFLEX_MAX $LFLEX_MED $LFLEX_MIN $LFLEX_SPR $GFLEX_MAX $GFLEX_MED $GFLEX_MIN $GFLEX_SPR $LRMSD $GRMSD $FNAT $FNONNAT $LOCAL_PCONS_MAX $LOCAL_PCONS_MED $LOCAL_PCONS_MIN $LOCAL_PCONS_SPR $LOCAL_PROQ3D_MAX $LOCAL_PROQ3D_MED $LOCAL_PROQ3D_MIN $LOCAL_PROQ3D_SPR"
