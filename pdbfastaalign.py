...
parser.add_option("-f", "--fas", dest="myfasfile", metavar = "FILE" ,help ="fasta file containing the sequence")
parser.add_option("-p", "--pdb", dest="mypdbfile" , metavar = "FILE",help ="pdb file")
parser.add_option("-c", "--chain", dest="mychain",  default = 'A' ,help="chain ID")
parser.add_option("-o", "--output", dest="myoutputfile",metavar = "FILE" , default = 'o.html' , help ="output html file")
parser.add_option("-O", "--outputpdb", dest="myoutputpdbfile", metavar = "FILE" , help ="output pdb file")
parser.add_option("-g", "--gapfill", dest="mygapopt", default = 'n', help ="option gapfill")
parser.add_option("-n", "--renumber", dest="mynumopt", default = 'y' , help ="option renumber")

(args, options) = parser.parse_args()

blastPath = '/homes/west/ncbi-blast-2.7.1+/bin/'
bl2seq_cmd = '%slegacy_blast.pl bl2seq -p blastp -i %s -j %s -F F -X 50'
...
<br>@ <a href="http://www.sdsc.edu" target="_blank">San Diego Supercomputer Center</a> (<a href="http://www.ucsd.edu" target="_blank">UCSD</a>)
  ...
  # print '\t--Calvin Chen\n\n'
   if not blastPath:
      ...
      ###
      # pfat_setup.py should set the python path
      # and also set the BLAST path

   fasFname, pdbFname, outFname, rnFname, gfFname, chainID = '', '' ,'', '', '', ''

   # ask for and verify FASTA file
   while(1):
   #   print '[path_to] FASTA filename:'
      fasFname = args.myfasfile #raw_input()
      if os.path.isfile(fasFname):
         break
    #  print '_Error_: %s is not a valid file.\n'%fasFname

   # ask for and verify PDB file
   while(1):
     # print '[path_to] PDB filename:'
      pdbFname = args.mypdbfile #raw_input()
      if os.path.isfile(pdbFname):
         break
     # print '_Error_: %s is not a valid file.\n'%pdbFname

   # ask for and verify chain ID
   while(1):
      #print 'ID of chain in PDB file to align with FASTA file:'
      chainID = args.mychain #raw_input()
      if chainID.isalpha():
   ...
   #print '[path_to] P-FAT html output file.'
   outFname = args.myoutputfile #raw_input()
   if not(outFname.endswith('.html') or outFname.endswith('.htm')):
      while(1):
    #     print 'Extend %s with ".html"? (y/n)'%outFname
         y_n = raw_input()
         if y_n == 'y':
   ...
   # option: re-number?
   while(1):
     # print 'Re-number chain %s with FASTA numeration? (y/n)'%chainID
      y_n = args.mynumopt #raw_input()
      if y_n == 'y':
         renum = True
         change = True
      #   print 'Please give a filename for the re-numbered PDB-file.'
         rnFname =  args.myoutputpdbfile #raw_input()
         if not rnFname.endswith('.pdb'):
            while(1):
       #        print 'Extend %s with ".pdb" (y/n)?'%rnFname
               y_n = raw_input()
               if y_n == 'y':
   ...
   # option: fill gaps?
   while(1):
      #print 'Fill gaps in chain %s? (y/n)'%chainID
      y_n = args.mygapopt #raw_input()
      if y_n == 'y':
         gapfill = True
         change = True
       #  print 'Please give a filename for the gap-filled PDB-file.'
         gfFname = raw_input()
         if not gfFname.endswith('.pdb'):
            while(1):
        #       print 'Extend %s with ".pdb" (y/n)?'%gfFname
               y_n = raw_input()
               if y_n == 'y':
...
#   sbjct = re.compile(r'^Sbjct:\s+(\d+)\s+([A-Z-]+)\s+(\d+)\s*$')
   ...
   for x in range(0,len(outs)):
      ...
      if theLine.find('Score = ') > 0:
         ...
         cutTo = x+3
   outs = outs[cutTo:]
   very_last_qpos = int(outs[-4][outs[-4].rfind(' '):].strip())
   very_last_spos = int(outs[-2][outs[-2].rfind(' '):].strip())
