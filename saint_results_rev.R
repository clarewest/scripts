...
saint_plot <- function(first_col,title,labels){
	plot(t$BEST_CT~t$BEST_RV,xlab="SAINT2 Reverse",ylab="SAINT2 Cotranslational",main=title,pch=21,xlim=c(0,1),ylim=c(0,1),col=first_col, bg=adjustcolor(first_col,alpha.f=tr),asp=1,cex=1.8)
	if (labels==1){
		left_labels=c("1V0D_A","1PIN","1RHS","1BSL_A","1BDC","1VII","2DN2_A","2A3D","1QQ3","1LMB_3","1T43")
		text(t$BEST_CT[! t$PDB %in% left_labels] ~t$BEST_RV[! t$PDB %in% left_labels], labels=t$PDB[! t$PDB %in% left_labels], cex=0.5, pos=4, offset=0.5)
		text(t$BEST_CT[t$PDB %in% left_labels] ~t$BEST_RV[t$PDB %in% left_labels], labels=t$PDB[t$PDB %in% left_labels], cex= 0.5, pos=2,offset=0.5)
	}
...
colour_plot <- function(sele,c){
	for (protein in sele){points(t[t$PDB==protein,4],t[t$PDB==protein,2],col=c,bg=adjustcolor(c,alpha.f=tr),pch=21,cex=1.8,xlim=c(0,1),ylim=c(0,1))}
}
...
s_multidomain <- c("1VCQ_C","1VCQ_N","1PIN_N","1V0D_A","1IBX","1T43_N")
...
png("~/pub_html/Clare/All_TMscores_CT_RV.png",width=2500,height=2500,res=300,pointsize=18)
par(pty="s")
...
for (protein in s_multidomain){points(t[t$PDB==protein,4],t[t$PDB==protein,2],col=cols[2],pch=1,cex=1.8)}
legend("bottomright",c("Multidomain","Domain of Multidomain"),pch=c(19,21),col=c(adjustcolor(cols[2],alpha.f=tr),cols[2]),cex=ce)
dev.off()

png("~/pub_html/Clare/Mechanism_TMscores_CT_RV.png",width=2500,height=2500,res=300,pointsize=18)
par(pty="s")
...
for (protein in rth$V1){points(t[t$PDB==protein,4],t[t$PDB==protein,2],col=cols[4],pch=1,cex=1.8)}
...
png("~/pub_html/Clare/CLass_TMscores_CT_RV.png",width=2500,height=2500,res=300,pointsize=18)
# Colour by class
par(pty="s")
...
png("~/pub_html/Clare/Length_TMscores_CT_RV.png",width=2500,height=2500,res=300,pointsize=18)
# Colour by length
par(pty="s")
