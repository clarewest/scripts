...
special = "--special" in args or "-s" in args   # Specially edited decoys to match discontinuous native structure
if crystal:
  args.remove("-c")
if homo:
  args.remove("-h")
...
chain = pdbcode[4]
if homo or saulo or rosetta:                                           # Scaffold chain is A for homology model or normal decoy
  decoychain = "A"
  if crystal:
...
if rev:
  ...
  score_residues = [True] * length           # Inclue all in scoring
else:
...
if homo:                                            # If seg is not from the native, it needs to be superimposed
  homo_scaf_residues = [False] * length
  if rev:
    scaf_range = tmhs[3:]
  else:
...
if saulo:
  ...
  decoys = [ decoyfile for decoyfile in glob.glob(decoysuperfolder + "/icarus.stats.ox.ac.uk/discontinousdecoys/" + pdbcode + "*linear*") if decoyfile[-4:] != ".cut" and decoyfile[-9:] != ".rescored" ]
else:
